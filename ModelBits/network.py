...
class network0(nn.Module):
    ...
    Outputs binary classification where 0 = NFS or UFS, 1 = CFS (as in the ClaimBuster Adversarial Transformer paper)
    '''
...
class network2(nn.Module):
    ...
    def forward(self, x):
        ...
        K1 = x[:,0,:].reshape((self.batch_size, self.input_size, 1))
        V1 = K1
        ...
        x2 = self.att2(Q,K2,V2)
        
        x = torch.cat([x1[0],Q,x2[0]],dim=1).squeeze()
        x = self.fc1(x)
        x = self.act(x)
        x = self.dropout(x)
        x = self.fc2(x)
